...
- Keeps prior fixes: tile-aligned stitching, info panel drawn last, caching, etc.
"""
...
UA = "FloodContextCV/1.2 (non-commercial; contact: example@local)"
...
# ---------------- Cache helpers ----------------
def cache_key(zoom: int, lat: float, lon: float) -> str:
    return f"zoom{zoom}_lat{lat:.5f}_lon{lon:.5f}"

def ensure_cache_dir(base: str, zoom: int, lat: float, lon: float) -> str:
    d = os.path.join(base, cache_key(zoom, lat, lon))
...
def stitch_tiles(lat: float, lon: float, zoom: int, radius: int, provider: str, verbose: bool):
    """
    Tile-aligned stitching (canvas origin = top-left tile boundary).
    Ensures tlx/tly (global px) match how tiles are pasted, so overlays align perfectly.
    """
    t0 = time.time()
    ...
    # Top-left TILE index used on the canvas
    ...
    # Canvas global pixel origin aligned to the top-left TILE
    ...
    with requests.Session() as session:
        ...
        # paste tiles in a 0..grid-1 loop so positions are exact multiples of TILE_SIZE
        for j in range(grid):
            for i in range(grid):
    ...
    for i in range(0, len(points), chunk):
        ...
        while True:
            try:
                ...
                time.sleep(delay)
        time.sleep(sleep_ms/1000.0)  # light rate limit
...
def safe_fill_nearest(elev_grid: np.ndarray, verbose: bool):
    """Fill NaNs using 4-neighbor propagation without broadcasting bugs."""
...
def colorize_risk01(risk01: np.ndarray) -> np.ndarray:
    """
    Green->Yellow->Red in BGR:
      0.0 -> (0,255,0)   0.5 -> (0,255,255)   1.0 -> (0,0,255)
    """
...
def detect_water_mask_bgr(bgr_img: np.ndarray) -> np.ndarray:
    """
    Returns a binary mask (uint8: 255 = water, 0 = land) using HSV + BGR heuristics.
    Works for both OSM standard and ESRI World Imagery reasonably well.
    """
    hsv = cv2.cvtColor(bgr_img, cv2.COLOR_BGR2HSV)
    H, S, V = hsv[...,0], hsv[...,1], hsv[...,2]
    B, G, R = bgr_img[...,0], bgr_img[...,1], bgr_img[...,2]

    # HSV water-ish range (cyan/blue)
    hsv_water = ((H >= 90) & (H <= 150) & (S >= 60) & (V >= 50))
    # BGR dominance for blue-ish pixels
    bgr_water = (B > (G + 10)) & (B > (R + 10)) & (B > 100)

    water = (hsv_water | bgr_water).astype(np.uint8) * 255

    # Morphological cleanup to remove speckles / thin channels that are not ocean
...
def masked_alpha_blend(base_bgr: np.ndarray, overlay_bgr: np.ndarray, alpha: float, keep_mask_uint8: np.ndarray) -> np.ndarray:
    """
    Per-pixel alpha blend but applied ONLY where keep_mask==255 (e.g., land).
    keep_mask==0 (e.g., ocean) leaves base image unchanged.
    """
...
def draw_vectors(img, elements, zoom, tlx, tly, verbose: bool,
    ...
    """
    Draw waterways, wetlands, embankments, and buildings.
    - Embankments: dark blue.
    - Buildings: red fill + red outline if mean risk >= bld_high_thr; otherwise gray.
    - High-risk buildings also get a SMALL, NON-OVERLAPPING RED DOT at their centroid.
    """
    t0 = time.time()
    node_lu = build_node_lookup(elements)
    ...
    # keep track of previously drawn dot centers to avoid overlaps
    placed_dots: List[Tuple[int,int]] = []
    dot_radius = max(1, int(dot_radius))
    dot_min_sep = max(dot_radius * 2 + 1, int(dot_min_sep))  # at least non-overlapping geometrically
    dot_min_sep2 = dot_min_sep * dot_min_sep  # squared
    ...
    # buildings (red fill + red edge if high risk) + non-overlapping RED DOT
...
def draw_legend(img, verbose: bool, dot_radius: int = 2):
    """
    Smaller box, bigger text, shows the red dot sample with the configured radius.
    """
    ...
    extras_h   = 5 * 18 + 16  # compact "Other layers" block
    padding    = 12
    ...
    # Bigger header text inside smaller box
    ...
    # Buildings (high-risk) + red dot sample (matches user-configured radius)
...
def main():
    ...
    ap.add_argument("--lat", type=float, default=14.8424, help="Center latitude")  # 12/14.8424/120.7948
    ap.add_argument("--lon", type=float, default=120.7948, help="Center longitude")
    ...
    # NEW: red dot controls
    ap.add_argument("--dot_radius", type=int, default=2,
                    help="Radius of red dot markers (px)")
    ap.add_argument("--dot_min_sep", type=int, default=6,
                    help="Minimum center-to-center separation (px) between red dots")
    ...
    # caching
    ap.add_argument("--cache_base", type=str, default="cache", help="Base cache directory")
    ap.add_argument("--no_cache", action="store_true", help="Bypass and ignore cache (always refetch)")
    ...
    # Prepare cache directory keyed by zoom/lat/lon
    cache_dir = ensure_cache_dir(args.cache_base, zoom, lat, lon)
    ...
    log(f"Cache folder: {cache_dir}", args.verbose)

    # ---------- Base map (from cache if available) ----------
    basemap_path = os.path.join(cache_dir, f"basemap_{args.provider}_r{args.radius}.png")
    cached = None if args.no_cache else load_img_if_exists(basemap_path)

    if cached is not None:
        img = cached.copy()
        grid = 2 * args.radius + 1
        w, h = TILE_SIZE * grid, TILE_SIZE * grid
        center_px, center_py = deg2xy(lat, lon, zoom)
        ...
        brx = tlx + w
        bry = tly + h
        log(f"Loaded basemap from cache (tile-aligned TL=({tlx},{tly}))", args.verbose)
    else:
        img, (tlx,tly), (brx,bry) = stitch_tiles(lat, lon, zoom, args.radius, args.provider, args.verbose)
        cv2.imwrite(basemap_path, img)
        log(f"Saved basemap to cache: {basemap_path}", args.verbose)
    ...
    # ---------- Overpass (from cache if available) ----------
    overpass_path = os.path.join(cache_dir, "overpass.json")
    elements = []
    if (not args.no_cache) and os.path.exists(overpass_path):
        try:
            over = read_json(overpass_path)
            elements = over.get("elements", [])
            log(f"Loaded Overpass from cache: {overpass_path} (elements={len(elements)})", args.verbose)
        except Exception as e:
            log(f"Failed to read cached Overpass: {e}", args.verbose)
    ...
    if not elements:
        try:
            ...
            log(f"Overpass fetch took {time.time()-t0:.2f}s (cached to {overpass_path})", args.verbose)
        except Exception as e:
            log(f"Overpass error: {e}", args.verbose)

    # ---------- Elevation (from cache if available) ----------
    elev_path = os.path.join(cache_dir, f"elev_pts_grid{args.grid}.json")
    elev_pts, lats, lons = [], [], []
    try:
        pts, lats, lons = build_sample_grid(bbox, args.grid, args.verbose)
        if (not args.no_cache) and os.path.exists(elev_path):
            try:
                elev_pts = read_json(elev_path)
                if not (isinstance(elev_pts, list) and (len(elev_pts)==0 or len(elev_pts[0])==3)):
                    raise ValueError("Bad elev cache format")
                log(f"Loaded elevation samples from cache: {elev_path} (n={len(elev_pts)})", args.verbose)
            except Exception as e:
                log(f"Bad elev cache, will refetch: {e}", args.verbose)
                elev_pts = []
        if not elev_pts:
            ...
            log(f"Saved elevation samples to cache: {elev_path}", args.verbose)
    except Exception as e:
        log(f"Elevation error: {e}", args.verbose)

    # ---------- Rain (from cache if available) ----------
    rain_path = os.path.join(cache_dir, "rain.json")
    rain_json = {}
    if (not args.no_cache) and os.path.exists(rain_path):
        try:
            rain_json = read_json(rain_path)
            log(f"Loaded Open-Meteo from cache: {rain_path}", args.verbose)
        except Exception as e:
            log(f"Failed to read cached rain: {e}", args.verbose)

    if not rain_json:
        try:
            rain_json = openmeteo_precip(lat, lon, args.verbose)
            write_json(rain_path, rain_json)
            log(f"Saved Open-Meteo to cache: {rain_path}", args.verbose)
        except Exception as e:
            log(f"Open-Meteo error: {e}", args.verbose)

    # ---------- Build land/water mask from the basemap ----------
    water_mask = detect_water_mask_bgr(img)        # 255 = water/ocean/sea, 0 = land
    land_mask  = cv2.bitwise_not(water_mask)       # 255 = land
    log(f"Water mask: {(water_mask>0).sum()} px marked as water.", args.verbose)

    # ---------- Risk overlay FIRST (but land-only) ----------
    risk_img_float = None
    if args.draw_risk and len(elev_pts)>0:
        try:
            ...
            log("Interpolate risk to full image...", args.verbose)
            ...
            # masked blend so ocean/sea stays clean (no overlay)
    ...
    # ---------- THEN vectors on top for visibility ----------
    ...
    # Save
    log(f"Saving -> {args.output}", args.verbose)
    ok = cv2.imwrite(args.output, save_img)
    log(f"Saved={ok}. File: {args.output}", args.verbose)
